...
class DatasourceResolver:
    ...
    def buildURLs(self, year, doy):
        ...
        url_json = ('https://{}{}{}/{:03d}.json'.format(self.SERVER_URL, self.TERRA_CMA, year, doy))
        r = requests.get(url_json)  # get the json file
        print(r.status_code)
        if r.status_code == 200:
            filename = r.json()[0]['name']
            url = ('https://{}{}{}/{:03d}/{}'.format(self.SERVER_URL,self.TERRA_CMA, year, doy, filename))
            urlList.append(url)

        # append TERRA CMG data (MOD09CMG)
        url_json = ('https://{}{}{}/{:03d}.json'.format(self.SERVER_URL, self.TERRA_CMG, year, doy))
        r = requests.get(url_json)  # get the json file
        print(r.status_code)
        if r.status_code == 200:
            filename = r.json()[0]['name']
            url = ('https://{}{}{}/{:03d}/{}'.format(self.SERVER_URL,self.TERRA_CMG, year, doy, filename))
            urlList.append(url)

        # append AQUA CMA data (MYD09CMA)
        url_json = ('https://{}{}{}/{:03d}.json'.format(self.SERVER_URL, self.AQUA_CMA, year, doy))
        r = requests.get(url_json)  # get the json file
        print(r.status_code)
        if r.status_code == 200:
            filename = r.json()[0]['name']
            url = ('https://{}{}{}/{:03d}/{}'.format(self.SERVER_URL, self.AQUA_CMA, year, doy, filename))
            urlList.append(url)

        # append AQUA CMG data (MYD09CMG)
        url_json = ('https://{}{}{}/{:03d}.json'.format(self.SERVER_URL, self.AQUA_CMG, year, doy))
        r = requests.get(url_json)  # get the json file
        print(r.status_code)
        if r.status_code == 200:
            filename = r.json()[0]['name']
            url = ('https://{}{}{}/{:03d}/{}'.format(self.SERVER_URL, self.AQUA_CMG, year, doy, filename))
            urlList.append(url)      
        print(urlList)
        return urlList
