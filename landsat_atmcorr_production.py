...
class LandsatAtmcorr():
    ...
    def check_process_status(self)->bool:
        ...
        for tmp_server in self.server_list:
            tmp_path = os.path.join(self.home_dir, tmp_server, 'landsat_sr', self.process_path.split('landsat/')[1])
    ...
    def check_process_data(self)->bool:
        ...
        if '/LC08/' in self.process_path:
            ...
            if len(tif_list)==10 and len(txt_list)==2:
                return True
            else:
                print(process__full_path, 'download has someting wrong!')
                return False
    ...
    def ymd_to_doy(self)->str:
        ...
        time_struct = datetime.strptime(process_date,'%Y%m%d').timetuple() # date convert to time struct
    ...
    def check_aux_data(self)->bool:
        ...
        if '/LE07/' in self.process_path:
            ...
            aux_rnls_path = os.path.join(self.ledaps_aux_dir, 'REANALYSIS/RE_'+year, rnls_file_name) 
            toms_file_name = 'TOMS_' + DOY + '.hdf' 
            aux_toms_path = os.path.join(self.ledaps_aux_dir, 'EP_TOMS/ozone_'+year, toms_file_name)
            ...
            aux_file_path = os.path.join(self.lasrc_aux_dir, 'LADS', year, aux_file_name)
    ...
    def atmos_correct_pre(self)->str:
        '''
        Function:
        ...
        local_path = os.path.join(self.home_dir, 'tq-tmp', 'landsat_sr', self.process_path.split('landsat/')[1])
        ...
        try:
            for tif in tif_list:
                ...
                ret1 = subprocess.run(['gdal_translate', '-co', 'TILED=NO', tif, os.path.join(local_path, tif_name)])
    ...
    def atmos_correct_process(self, local_path:str)->bool:
        ...
        # check the MTL.txt
        mtl_txt = glob.glob(os.path.join(local_path, '*_MTL.txt'))[0]
        print(mtl_txt)
        # convert lpgs to espa
        os.chdir(local_path) #  must be change
        ret1 = subprocess.run(['convert_lpgs_to_espa', '--mtl', mtl_txt, '--del_src_files'])
        if (ret1.returncode != 0):
        ...
        # atomspheric correct
        mtl_xml = glob.glob(os.path.join(local_path, '*.xml'))[0] # read XML
        if 'LC08' in mtl_xml:
    ...
    def atmos_correct_post(self, local_path:str)->str:
        '''
        Function:
        ...
        # check the XML
        xml = glob.glob(os.path.join(local_path, '*.xml'))[0]
        ...
        if (ret1.returncode == 0):
            ...
            ret1 = subprocess.run(['dilate_pixel_qa', '--xml', xml ,'--bit=5', '--distance=3'])
            if (ret1.returncode == 0):
        ...
        ret1 = subprocess.run(['convert_espa_to_gtif', '--xml',  xml, '--gtif', base_name, '--del_src_files'])
        if (ret1.returncode != 0):
    ...
    def go_process(self)->int:
        ...
        # Step 4
        local_path = self.atmos_correct_pre()
        ...
        # step 6
        result_path = self.atmos_correct_post(local_path) 
        if not flag:
            process_status = 6
            return None, process_status
        else:
...
if __name__ == '__main__':
    start = time.time()
    #process_path = '/home/jason/tq-data02/landsat/LE07/01/013/028/LE07_L1GT_013028_20100117_20160916_01_T2'
    #process_path = '/home/jason/tq-data02/landsat/LE07/01/046/029/LE07_L1TP_046029_20061012_20160923_01_T1'
    #process_path = '/home/jason/tq-data02/landsat/LC08/01/057/020/LC08_L1TP_057020_20140430_20170307_01_T1'
    #process_path = '/home/jason/tq-data02/landsat/LC08/01/014/031/LC08_L1GT_014031_20131226_20170307_01_T2'
    #process_path = '/home/jason/tq-data02/landsat/LT05/01/029/028/LT05_L1TP_029028_19841101_20161004_01_T1'
    process_path = '/home/jason/tq-data03/landsat/LT05/01/013/028/LT05_L1GS_013028_20100922_20160831_01_T2'

    result_root = 'tq-data02' 
