...
def find_error_raw():
    ...
    result_json = "/home/tq/data_pool/test_data/landsat/tif_list.json"
    with open(result_json, "r") as fp:
        processed_list = json.load(fp)

    for path in processed_list:
        file_name = path.split("landsat_sr/")[1]
        path = os.path.join("/home/tq", "*", "landsat", file_name)
        print(path)
        path_name = glob.glob(os.path.join("/home/tq/*/landsat/", file_name))
        print(path_name)
